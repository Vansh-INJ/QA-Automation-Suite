...
var getCommandRuntimeEnv = () => {
  const values = Cypress.expose() ?? {};
  return {
    visualRegressionType: values.visualRegressionType,
    visualRegressionBaseDirectory: values.visualRegressionBaseDirectory,
    visualRegressionDiffDirectory: values.visualRegressionDiffDirectory,
    visualRegressionGenerateDiff: values.visualRegressionGenerateDiff,
    visualRegressionFailSilently: values.visualRegressionFailSilently,
    visualRegressionUpdateSnapshots: values.visualRegressionUpdateSnapshots
  };
};
...
function prepareOptions(name, screenshotOptions, commandOptions, configEnv) {
  if (configEnv.visualRegressionType === void 0) {
    throw new Error(
  ...
  if (configEnv.visualRegressionBaseDirectory !== void 0) {
    options.baseDirectory = configEnv.visualRegressionBaseDirectory;
  }
  if (configEnv.visualRegressionDiffDirectory !== void 0) {
    options.diffDirectory = configEnv.visualRegressionDiffDirectory;
  }
  if (configEnv.visualRegressionGenerateDiff !== void 0) {
    options.generateDiff = configEnv.visualRegressionGenerateDiff;
  }
  if (configEnv.visualRegressionFailSilently !== void 0) {
    options.pluginOptions.failSilently = configEnv.visualRegressionFailSilently;
  }
  if (configEnv.visualRegressionUpdateSnapshots !== void 0) {
    const envValue = configEnv.visualRegressionUpdateSnapshots;
